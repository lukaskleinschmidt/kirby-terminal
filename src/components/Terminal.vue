<template>
  <section
    v-if="gate === true && isLoading === false"
    :class="['terminal-section', theme]"
  >
    ...
    <k-dialog
      ...
      @submit="submit"
    >
      <k-text v-if="dialog.text" v-html="dialog.text" />
      <k-form
        v-if="dialog.fields"
        v-model="payload"
      ...
      <template v-if="error && error.key === 'error.terminalDialog'">
        <dl
          v-if="error.details && Object.keys(error.details).length"
